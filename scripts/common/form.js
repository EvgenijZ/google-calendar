...
export const prepareInputsForModal = () => {
    ...
    const calendarTimeSlot = document.querySelector('.calendar__time-slot[data-slot-date]');
    ...
    switch (true) {
        case date.format("mm") >= 45:
            if (+inputStartHours.value === 23) {
                ...
                setTimesForInputs(+inputStartHours.value + 1, '00', +inputEndHours.value + 1, '00');
            }
            break;
...
const loadInputsEditModal = () => {
    ...
    getEvent(currentEventId).then((event) => {
        ...
        setTimesForInputs(moment(event.start).format("HH"), moment(event.start).format("mm"),
            moment(event.end).format("HH"), moment(event.end).format("mm"));
